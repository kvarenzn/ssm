...
4. 使用命令`./ssm -d {难度} -n {歌曲ID} -r {旋转方向}`启动ssm
	...
	- 如果一切正常，控制台（或命令提示符）会输出`ENTER/SPACE GO!!!!!`
5. 在游戏设备中选中对应的曲目和难度，进入打歌界面
6. 当第一个音符快到判定线时，在运行了ssm的控制台内敲下回车或者空格
7. 若一切正常，ssm会自动完成曲目剩余的部分。
8. 如果你发现在击打音符时晚了或早了，可以在控制台按左/右方向键调节当前偏移。
    - 左方向键：偏移-10ms；右方向键：偏移+10ms
    - 按方向键时按住`Shift`可以每次加减50ms，按住`Ctrl`可以每次加减100ms
    - 早了（音符还没有落到判定线就被击打）：按左方向键
    - 晚了（音符已经落到判定线但还未击打）：按右方向键
9. 若希望中断自动打歌，可以直接在控制台（或命令提示符）中输入`Ctrl-C`（Windows下是`Ctrl-Z`）强制终止程序
